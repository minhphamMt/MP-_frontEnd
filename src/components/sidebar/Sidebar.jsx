...
export default function Sidebar({ isOpen, onClose, isArtistWorkspace = false }) {
  ...
  const compactToggleClassName = isArtistTone
    ? "group/compact-toggle relative hidden h-12 w-12 items-center justify-center overflow-hidden rounded-[18px] border border-sky-200/[0.14] bg-[#152133] shadow-[0_12px_24px_rgba(2,6,18,0.34)] transition duration-300 lg:flex md:hover:border-sky-200/[0.24] md:hover:bg-[#20304a]"
  ...
  const compactToggleGlowClassName = isArtistTone
    ? "pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(132,190,232,0.16),transparent_55%)] opacity-0 transition duration-300 group-hover/compact-toggle:opacity-100 group-focus-visible/compact-toggle:opacity-100"
  ...
  const collapseButtonClassName = isArtistTone
    ? "absolute right-3 top-1/2 hidden -translate-y-1/2 items-center justify-center rounded-full border border-sky-200/[0.14] bg-[#182337] p-2 text-slate-100/80 transition lg:flex pointer-events-none opacity-0 md:group-hover/sidebar:pointer-events-auto md:group-hover/sidebar:opacity-100 md:group-focus-within/sidebar:pointer-events-auto md:group-focus-within/sidebar:opacity-100 md:hover:border-sky-200/[0.3] md:hover:bg-[#22324d] md:hover:text-white"
  ...
  const closeButtonClassName = isArtistTone
    ? "rounded-full border border-sky-200/[0.14] bg-[#182337] p-2 text-slate-100/82 transition lg:hidden md:hover:border-sky-200/[0.3] md:hover:bg-[#22324d] md:hover:text-white"
  ...
  return (
    <>
      <div
        className={`sidebar-overlay-motion fixed inset-0 z-[60] bg-black/65 backdrop-blur-[2px] transition-[opacity,backdrop-filter] duration-300 ease-out lg:hidden ${
      ...
      <aside
        ...
        className={`sidebar-motion group/sidebar fixed inset-y-0 left-0 z-[70] flex h-full w-[276px] sm:w-[304px] flex-col overflow-hidden border-r text-white will-change-transform transition-[width,transform] duration-[380ms] ease-[cubic-bezier(0.22,1,0.36,1)] lg:static lg:z-auto lg:overflow-visible lg:translate-x-0 lg:duration-[280ms] ${sidebarShellClassName} ${
	          ...
	          <span
	            className={`h-28 w-px rounded-full transition ${
	              isResizing
	                ...
	                : isArtistTone
	                  ? "bg-sky-200/[0.18] opacity-0 group-hover/sidebar:opacity-100 group-focus-within/sidebar:opacity-100"
