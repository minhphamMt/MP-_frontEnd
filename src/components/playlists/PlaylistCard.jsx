...
export default function PlaylistCard({ playlist, onOpen }) {
  ...
  return (
    <button
      ...
      className="group relative w-52 shrink-0 overflow-hidden rounded-2xl border border-white/10
      ...
      <div className="relative w-full overflow-hidden rounded-xl">
        {cover ? (
          <img
            ...
            className="h-44 w-full rounded-xl object-cover transition-transform duration-500 group-hover:scale-[1.05]"
          />
        ) : (
          <div className="flex h-44 w-full items-center justify-center rounded-xl bg-gradient-to-br from-slate-700 to-slate-900 text-4xl text-white/40">
            🎵
          </div>
        )}
