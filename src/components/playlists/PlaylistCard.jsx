...
export default function PlaylistCard({ playlist, onOpen, variant = "grid" }) {
  ...
  return (
    <button
      ...
      className={`group relative w-full overflow-hidden text-left p-3 transition-all duration-300 sm:w-60 sm:shrink-0 sm:p-4 lg:w-64 ${
        isLibrary ? "rounded-lg border border-white/10 bg-[#181818]" : "user-surface"
      }`}
    >
      <div className={`relative w-full overflow-hidden ${isLibrary ? "rounded-md" : "rounded-xl"}`}>
        {cover ? (
          <OptimizedImage
            src={cover}
          ...
          <div
            className={`flex h-36 w-full items-center justify-center text-4xl text-white/40 sm:h-44 lg:h-52 ${
              isLibrary ? "rounded-md bg-[#232323]" : "rounded-xl bg-[#232323]"
            }`}
          >
            ♪
          </div>
      ...
      <div className="relative mt-3 space-y-1">
        <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.2em] text-white/50">
          <FiList className="text-emerald-300" />
          Playlist
        </div>
        ...
        <div className="flex items-center gap-2 text-xs text-white/70 sm:text-sm">
          <FiMusic className="shrink-0 text-emerald-300" />
